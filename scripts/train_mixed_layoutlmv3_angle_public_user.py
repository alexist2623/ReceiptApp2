...
from ml.angle_geometry import angle_feature_dim_for_mode, normalize_angle_encoding_mode
...
from scripts.train_mixed_layoutlmv3_public_user import (
    box_from_norm,
    collect_user_pairs,
    load_cord_records,
    load_extra_jsonl_records,
    load_jsonl,
    load_public_bio_records,
    plot_history,
    repeat_records,
    save_json,
    split_user_pairs,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune angle-aware LayoutLMv3 on CORD + rotated WildReceipt + hand-labeled data."
    )
    ...
    parser.add_argument("--wildreceipt_bio_dir", default="processed_data/wildreceipt_rotated_receipt_v2_bio")
    parser.add_argument("--wild_bio_dir", default=None)
    parser.add_argument("--user_input_dir", action="append", default=None)
    ...
    parser.add_argument("--output_dir", default="models/layoutlmv3-angle-mixed-public-user")
    parser.add_argument("--sources", default="cord,wild,user")
    ...
    parser.add_argument("--angle_feature_dim", type=int, default=None)
    parser.add_argument("--angle_hidden_size", type=int, default=128)
    parser.add_argument("--angle_fusion", default="add")
    parser.add_argument("--angle_dropout", type=float, default=0.1)
    parser.add_argument("--max_abs_angle_deg", type=float, default=45.0)
    ...
    parser.add_argument(
        "--best_metric",
        choices=("mixed_validation_seqeval_f1", "custom_validation_seqeval_f1"),
        default="mixed_validation_seqeval_f1",
        help="Metric used to save output_dir/best. Default uses the full mixed validation F1.",
    )
...
class MixedAnglePublicUserDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if source == "cord":
            ...
            if not image_path:
                raise ValueError(f"BIO record has no image path: {record.get('id')}")
        ...
        payloads = record.get("word_payloads") or [{} for _ in words]
        keep_words = []
        keep_boxes = []
        keep_normalized = []
        keep_labels = []
        keep_payloads = []
        for word, box, norm, label, payload in zip(words, boxes, normalized_boxes, labels, payloads):
            ...
            keep_payload = dict(payload) if isinstance(payload, dict) else {}
            keep_payload.setdefault("text", str(word))
            keep_payload.setdefault("box", box)
            keep_payloads.append(keep_payload)
        if not keep_words:
...
def collate_fn(processor, args):
    mode = normalize_angle_encoding_mode(args.angle_encoding_mode)
    feature_dim = int(args.angle_feature_dim or angle_feature_dim_for_mode(mode))
    ...
    def collate(samples):
        encoding = encoding_with_angle_features(
            ...
            max_abs_angle_deg=args.max_abs_angle_deg,
        )
        encoding["record_ids"] = [sample["id"] for sample in samples]
        encoding["sources"] = [sample["source"] for sample in samples]
        encoding["angle_stats"] = [
            {
                "num_words_with_angle": sample.get("num_words_with_angle", 0),
                "num_words_without_angle": sample.get("num_words_without_angle", 0),
            }
...
def metric_from_sequences(true_sequences, pred_sequences):
    return {
...
def evaluate(model, loader, device, id2label):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for stats in batch.get("angle_stats", []):
                angle_words["with_angle"] += int(stats.get("num_words_with_angle", 0))
                angle_words["without_angle"] += int(stats.get("num_words_without_angle", 0))
    ...
    metrics = {
        ...
        "angle_word_counts": dict(angle_words),
        **metric_from_sequences(true_sequences, pred_sequences),
    }
    metrics["source_metrics"] = {
        source: metric_from_sequences(per_source_true[source], per_source_pred[source])
...
def main():
    ...
    angle_mode = normalize_angle_encoding_mode(args.angle_encoding_mode)
    angle_feature_dim = int(args.angle_feature_dim or angle_feature_dim_for_mode(angle_mode))
    ...
    if "wild" in enabled_sources and wild_bio_dir:
        wild_train = load_public_bio_records(wild_bio_dir, "train", "wildreceipt_rotated", args.max_wild_train_samples)
        wild_eval = load_public_bio_records(wild_bio_dir, "validation", "wildreceipt_rotated", args.max_public_eval_samples or args.max_wild_eval_samples)
    ...
    print(f"angle_encoding_mode: {angle_mode}")
    ...
    model = load_angle_aware_token_classifier(
        ...
        angle_hidden_size=args.angle_hidden_size,
        angle_dropout=args.angle_dropout,
        angle_fusion=args.angle_fusion,
        angle_encoding_mode=angle_mode,
    )
    model.to(device)
    train_loader = DataLoader(
        MixedAnglePublicUserDataset(train_records, label2id, cord_raw_dataset),
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=0,
        collate_fn=collate_fn(processor, args),
    )
    eval_loader = DataLoader(
        MixedAnglePublicUserDataset(eval_records, label2id, cord_raw_dataset),
    ...
    if custom_test_records:
        custom_test_loader = DataLoader(
            MixedAnglePublicUserDataset(custom_test_records, label2id, cord_raw_dataset),
            batch_size=args.eval_batch_size,
            shuffle=False,
            num_workers=0,
            collate_fn=collate_fn(processor, args),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(tqdm(train_loader, desc=f"angle mixed epoch {epoch:03d}/{args.epochs:03d}", unit="batch"), start=1):
        ...
        row = {
            ...
            "angle_word_counts": eval_metrics.get("angle_word_counts", {}),
            "source_metrics": eval_metrics["source_metrics"],
            "best_so_far": False,
        }
        custom_val_f1 = row["source_metrics"].get("user", {}).get("seqeval_f1")
        if args.best_metric == "custom_validation_seqeval_f1" and custom_val_f1 is not None:
            selection_f1 = custom_val_f1
            selection_metric_name = "custom_validation_seqeval_f1"
        else:
            selection_f1 = row["seqeval_f1"]
            selection_metric_name = "mixed_validation_seqeval_f1"
        row["selection_metric"] = selection_f1
        row["selection_metric_name"] = selection_metric_name
        if selection_f1 > best_f1:
            best_f1 = selection_f1
            best_epoch = epoch
            best_metrics = eval_metrics
            row["best_so_far"] = True
            save_angle_aware_model_bundle(output_dir / "best", model, processor, labels_payload)
        history.append(row)
        print(
    ...
    save_angle_aware_model_bundle(output_dir / "last", model, processor, labels_payload)
    ...
    save_json(
        output_dir / "split_manifest.json",
        {
            ...
            "notes": [
                "CORD records have no native angle/quad data and receive zero angle features.",
                "Rotated WildReceipt records use word_payloads[].quad and angle_deg generated by the converter.",
            ],
        },
    )
    ...
    print("Angle-aware mixed public + user LayoutLMv3 fine-tuning passed.")
