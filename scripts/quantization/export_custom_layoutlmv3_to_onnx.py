...
def parse_args():
    parser = argparse.ArgumentParser(description="Export custom receipt LayoutLMv3 token classifier to ONNX FP32.")
    parser.add_argument("--checkpoint", default="models/layoutlmv3-item-policy-mixed-100/best")
    parser.add_argument("--input_dir", required=True, nargs="+")
    parser.add_argument("--exclude_dir_name", default="Temp")
    parser.add_argument("--sample_index", type=int, default=0)
    parser.add_argument("--out_dir", default="models/layoutlmv3-item-policy-onnx/fp32")
...
def main():
    ...
    encoding = encode_custom_sample(processor, sample, args.max_length, include_labels=False)
    ...
    print(f"model size MB: {onnx_total_size_mb(onnx_path):.2f}")
    ...
    if not smoke["passed"]:
        fail("Custom ONNX FP32 export smoke test failed.")
    print("Custom LayoutLMv3 ONNX FP32 export passed.")
