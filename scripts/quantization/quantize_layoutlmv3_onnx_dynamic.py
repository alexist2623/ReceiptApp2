...
def parse_args():
    parser = argparse.ArgumentParser(description="Apply ONNX Runtime dynamic INT8 quantization to LayoutLMv3 ONNX.")
    ...
    parser.add_argument("--out_dir", default="models/layoutlmv3-cord-onnx/int8_dynamic")
...
def main():
    ...
    try:
        ...
        quant_warning = f"per_channel/op_types fallback after TypeError: {exc}"
    ...
    smoke = {
        ...
        and not finite_report(outputs)["logits"]["has_nan"]
        and not finite_report(outputs)["last_hidden_state"]["has_nan"],
    }
    ...
    save_json(
        out_dir / "model_size_report.json",
        {
            "input_size_mb": input_size,
            "output_size_mb": output_size,
            "size_reduction_percent": reduction,
    ...
    if not smoke["passed"]:
        fail("Dynamic INT8 ONNX smoke test failed.")
    print("LayoutLMv3 ONNX dynamic INT8 quantization passed.")
