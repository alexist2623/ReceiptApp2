...
def binary_edge_metrics(labels, probs, threshold=0.84):
...
def field_edge_metrics(samples, probs_by_sample, threshold=0.84, fields=None):
    if fields is None:
        fields = DEP_FIELDS
...
def _edge_sets(sample, probs, threshold=0.84, dep_field="MENU_PRICE"):
...
def menu_price_pair_metrics(samples, probs_by_sample, threshold=0.84):
    return item_price_pair_metrics(samples, probs_by_sample, threshold)


def item_price_pair_metrics(samples, probs_by_sample, threshold=0.84):
    tp = fp = fn = 0
    for sample, probs in zip(samples, probs_by_sample):
...
def fields_pair_metrics(samples, probs_by_sample, fields, threshold=0.84):
...
def summary_amount_pair_metrics(samples, probs_by_sample, threshold=0.84):
...
def hard_negative_false_positives(samples, probs_by_sample, threshold=0.84):
    count = 0
    store_count = 0
...
def dependent_collision_count(samples, probs_by_sample, threshold=0.84):
    total = 0
    for sample, probs in zip(samples, probs_by_sample):
        selected = Counter()
        for idx, meta in enumerate(sample.get("pair_meta", [])):
...
def aggregate_metrics(samples, probs_by_sample, threshold=0.84):
    labels = []
    probs = []
